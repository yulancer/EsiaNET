...
namespace ESIA.AspNetIdentityExample
{
    ...
    public class Esia
    {
        public static EsiaOptions GetOptions()
        {
            ...
            {
                ClientId = "REGRDCPIR",    // Ваш идентификатор системы-клиента
                RedirectUri = EsiaConsts.EsiaAuthUrl,   // Адрес перенаправления на страницу предоставления прав доступа в ЕСИА - либо тестовый, либо рабочий
                TokenUri = EsiaConsts.EsiaTokenUrl,     // https-адрес ЕСИА для получения маркера доступа - либо тестовый, либо рабочий
                RestUri = EsiaConsts.EsiaRestUrl,       // Адрес REST-сервиса ЕСИА для получения данных - либо тестовый, либо рабочий
